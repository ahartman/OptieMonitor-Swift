...
@Observable class ViewModel {
    ...
    func formatGraph(lines: [IncomingLine], sender: String = "") -> (
        ...
        let indexRounding = 10.0
        var indexMax =
            localGraphLines.filter { $0.type == "Index" }.map { $0.waarde }
            .max() ?? 0
        indexMax = max(indexMax, captions.strike1, captions.strike2)
        indexMax =
            (indexMax / indexRounding).rounded(.up) * indexRounding
        var indexMin =
            localGraphLines.filter { $0.type == "Index" }.map { $0.waarde }
            .min() ?? 0
        indexMin = min(indexMin, captions.strike1, captions.strike2)
        indexMin =
            (indexMin / indexRounding).rounded(.down) * indexRounding

        let localyValues = [
            "Euro": Array(stride(from: yMin, through: yMax, by: rounding)),
            "Index": Array(
                stride(from: indexMin, through: indexMax, by: indexRounding)
            ),
        ]
    ...
    func formatList(lines: [IncomingLine]) -> [TableLine] {
        ...
        for (index, line) in lines.enumerated() {
            ...
            let factor = 3.0
            let valueRange = line.callValue / factor...line.callValue * factor
            if !valueRange.contains(line.putValue) {
                lineFormatted.warningColor = .red
            }
    ...
    func unpackJSON(result: IncomingData) {
        let temp1 = result.caption.components(separatedBy: "-")
        let temp2 = temp1[1].components(separatedBy: ",")
        captions = Captions(
            caption: result.caption,
            strike1: Double(temp2[0].filter { ("0"..."9").contains($0) }) ?? 0,
            strike2: Double(temp2[1].filter { ("0"..."9").contains($0) }) ?? 0
        )

        intraday.list = formatList(lines: result.intradays)
        intraday.footer = formatFooter(
            lines: result.intradays,
            openLine: result.interdays.first!,
            sender: "intra"
        )
    ...
    func getJsonData(action: String) async {
        ...
        do {
            ...
            let decodedData = try decoder.decode(IncomingData.self, from: data)
            unpackJSON(result: decodedData)
    ...
    struct Expiries: Decodable {
        let expiry1970: Int
        let expiry: String
    }

    func getJsonTest(action: String) async {
        let url = URL(string: dataURL + action)!
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        do {
            let (incomingData, _) = try await URLSession.shared.data(from: url)
            /*
